function latex = latexTable2(input)
% Input:
...
% Define what should happen with NaN values in input.tableData:
if ~isfield(input,'bigTable'),input.bigTable = 0;end
...
if rowLabelsExist
    if(isstring(input.tableRowLabels)) 
        input.tableColLabels=cellstr(input.tableRowLabels);
    end
    C(1+colLabelsExist:end,1)=input.tableRowLabels';
end
if colLabelsExist
    if(isstring(input.tableColLabels)) 
        input.tableColLabels=cellstr(input.tableColLabels);
    end
...
hLine = '\hline';
if(length(input.tableColumnAlignment)==size(C,2))
    header = ['\begin{tabular}','{',input.tableColumnAlignment,'}'];
else
    header = ['\begin{tabular}','{l',repmat(input.tableColumnAlignment,1,size(C,2)-1),'}'];
    end
% Make header
if(input.bigTable==1)
    latex = {['\begin{table}',input.tablePlacement];'\centering';...
            '\resizebox{\textwidth}{!}{';['\caption{',input.tableCaption,'}']; ...
            ['\label{table:',input.tableLabel,'}'];header;hLine};
else
    latex = {['\begin{table}',input.tablePlacement];'\centering';...
            ['\caption{',input.tableCaption,'}'];['\label{table:',input.tableLabel,'}'];header;hLine};
end


for i=1:size(C,1)
    if i==2 
        latex(end+1) = {strcat(hLine,hLine)};
    end
    rowStr = '';
    for j=1:size(C,2)
        % Extract value in cell i-j
        dataValue = C{i,j};
        % Adjust depending on value 
        if iscell(dataValue)
          dataValue = dataValue{:};
        ...
        % If first column add text
        if j==1
            rowStr = dataValue;
        else
            % Latter rows separate by 
